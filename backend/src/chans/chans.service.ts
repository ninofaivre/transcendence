...
type ChanPayload = Prisma.ChanGetPayload<{ select: ReturnType<ChansService["getChansSelect"]> }>
...
export class ChansService {
	...
	public getDoesUserHasSelfPermSelect = (
		username: string,
	...
	private formatChanUser = (user: ChanPayload["users"][number]) =>
		({
			name: user.name,
	...
	private formatChan(chan: ChanPayload) {
		...
		const selfPerms = [
			...new Set(
				roles.flatMap((el) =>
					el.permissions.filter(
						(el): el is z.infer<typeof zSelfPermissionList> =>
	...
	private formatChanArray = (chans: ChanPayload[]) => chans.map((chan) => this.formatChan(chan))
	...
	private formatChanDiscussionMessageForUser(
		...
		if (element.event) {
			const {
				event: {
					deletedMessageChanDiscussionEvent: { deletingUserName },
				},
				authorName: author,
				...elementRest
			} = element
		...
		return {
			...
			relatedTo: related && {
				...
				preview: (() => {
					if (related.event) {
						const retypedRelatedEvent = related.event as ChanRetypedEvent<
							typeof related.event
						>
						if (retypedRelatedEvent.deletedMessageChanDiscussionEvent)
							...
							return {
								type: "event",
								eventType: retypedRelatedEvent.classicChanDiscussionEvent.eventType,
							} as const
					}
					return {
						type: "message",
			...
			...messageRest,
			mentionMe: !!(
				this.namesArrayToStringArray(relatedRoles.concat(relatedUsers)).includes(
					username,
				) ||
			...
			hasBeenEdited:
				element.creationDate.getTime() !== element.message.modificationDate.getTime(),
			type: "message",
		} as const
	}
	...
	private formatChanDiscussionElementForUser(
		...
		if (event) return this.formatChanDiscussionEvent(username, { ...element, event })
	...
	private formatChanDiscussionElementArrayForUser(
		...
		return elements.map((element) => this.formatChanDiscussionElementForUser(username, element))
	}
	...
	public async doesUserHasSelfPermInChan(
		...
		return !!(
			...
			roles.some(
				(el) =>
					el.users.map((el) => el.name).includes(username) &&
	...
	public doesUserHasPermOverUserInChan(
		...
		if (username === otherUserName || username === ownerName) return true
		if (
			roles.some((role) => {
				if (
					role.roleApplyOn === "NONE" ||
					role.users.every((user) => user.name !== username) ||
					role.permissions.every((el) => el !== perm)
				)
					return false
				if (
					role.roles.some((el) => el.users.some((el) => el.name === otherUserName)) &&
					role.users.every((user) => user.name !== otherUserName)
				)
					return true
				if (
					role.roleApplyOn === "ROLES_AND_SELF" &&
	...
	async leaveChan(username: string, chanId: string) {
		...
		if (!chan) return contractErrors.NotFoundChan(chanId)
		if (username === chan.ownerName) return contractErrors.OwnerCannotLeaveChan()
	...
	getAtsFromChanMessageContent(
		...
		const uncheckedAts = content
			.split(" ")
			.filter((el) => el.startsWith("@"))
			.flatMap((el) => el.split("@"))
		return {
			users: chan.users.filter((user) => uncheckedAts.includes(user.name)),
			roles: chan.roles.filter((role) => uncheckedAts.includes(role.name)),
		}
	}
	...
	async createChanMessageIfRightTo(
		...
		const chan = await this.getChan(
			{ id: chanId, users: { some: { name: username } } },
			{
				...
				roles: {
					select: {
						...this.getDoesUserHasSelfPermSelect(username, "SEND_MESSAGE").roles.select,
						name: true,
					},
				},
				...(relatedTo
		...
		const newMessage = await this.createChanMessage(username, chanId, content, relatedTo, ats)
	...
	async updateChanMessage(
		username: string,
		{ chanId, messageId }: RequestShapes["updateChanMessage"]["params"],
		content: string,
	) {
		const chan = await this.getChan(
			{ id: chanId, users: { some: { name: username } } },
			{
				...
				elements: {
					where: { id: messageId, message: { isNot: null } },
					select: {
						authorName: true,
						message: {
							select: {
		...
		if (!chan.elements.length || !chan.elements[0].message)
			return contractErrors.NotFoundChanMessage(chanId, messageId)
		const oldMessage = chan.elements[0].message
		if (!(await this.doesUserHasSelfPermInChan(username, "SEND_MESSAGE", chan)))
			return contractErrors.ChanPermissionTooLow(username, chanId, "SEND_MESSAGE")
		if (chan.elements[0].authorName !== username)
			return contractErrors.NotOwnedChanMessage(username, "update", messageId, chanId)
		const ats = this.getAtsFromChanMessageContent(chan, content)
			...
			data: {
				message: {
					update: {
						content: content,
						relatedUsers: {
							connect: ats.users.filter((el) =>
								oldMessage.relatedUsers.every((user) => user.name !== el.name),
							),
							disconnect: oldMessage.relatedUsers.filter((user) =>
								ats.users.every((el) => el.name !== user.name),
							),
						},
						relatedRoles: {
							connect: ats.roles
								.filter((el) =>
									oldMessage.relatedRoles.every((role) => role.name !== el.name),
								)
		...
		if (!message) return contractErrors.ContentModifiedBetweenUpdateAndRead("ChanMessage")
		chan.users.forEach(({ name }) => {
			this.sse.pushEvent(name, {
	...
	private async deleteChanMessage(elementId: string, deletingUserName: string) {
		...
		const retypedDeletedElement = deletedElement as ChanRetypedElementTest<
			typeof deletedElement
		>
	...
	async deleteChanMessageIfRightTo(
		username: string,
		{ chanId, messageId }: RequestShapes["deleteChanMessage"]["params"],
	) {
		const chan = await this.getChan(
			{ id: chanId, users: { some: { name: username } } },
			{
		...
		if (!chan) return contractErrors.NotFoundChan(chanId)
		if (!chan.elements.length) return contractErrors.NotFoundChanMessage(chanId, messageId)
		const { authorName } = chan.elements[0]
		if (!this.doesUserHasPermOverUserInChan(username, authorName, chan, "DELETE_MESSAGE"))
			return contractErrors.ChanPermissionTooLowOverUser(
				username,
				authorName,
				chanId,
				"DELETE_MESSAGE",
			)
		const deletedElement = await this.deleteChanMessage(messageId, username)
		if (isContractError(deletedElement)) return deletedElement
		chan.users
	...
	public async createAndNotifyClassicChanEvent(
		...
		newEvent.chan.users.forEach(({ name }) => {
			this.sse.pushEvent(name, {
				...
				data: {
					chanId,
					element: this.formatChanDiscussionElementForUser(
						name,
	...
	public async pushUserToChanAndNotifyUsers(username: string, chanId: string) {
		...
		if (!newUser) return contractErrors.ContentModifiedBetweenCreationAndRead("ChanUser")
	...
	async joinChanById(
		username: string,
		{ chanId, password }: RequestShapes["joinChanById"]["body"],
	) {
		...
		if (!res) return contractErrors.NotFoundChan(chanId)
		const { password: chanPassword, users } = res
		if (users.length) return contractErrors.ChanUserAlreadyExist(username, chanId)
		if (password && !chanPassword) return contractErrors.ChanDoesntNeedPassword(chanId)
		if (!password && chanPassword) return contractErrors.ChanNeedPassword(chanId)
		if (chanPassword && password && !compareSync(password, chanPassword))
			return contractErrors.ChanWrongPassword(chanId)
		await this.chanInvitationsService.updateAndNotifyManyInvsStatus("ACCEPTED", {
			chanId,
