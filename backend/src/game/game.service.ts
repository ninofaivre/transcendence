...
import {
	...
	IntraUserName,
} from "src/websocket/game.websocket.gateway"
...
abstract class GameObject {
	...
	protected doesBallCollideWithPaddle = (ball: Rectangle, paddle: Rectangle) =>
		...
		ball.topY <= paddle.botY &&
		ball.botY >= paddle.topY
...
class Paddle extends GameObject {
	...
	private set position(newPosition: Position) {
		...
		if (this.getRect().topY < 0) newPosition.y = Paddle.yOffset
		else if (this.getRect().botY > GameDim.court.height)
			newPosition.y = GameDim.court.height - Paddle.yOffset
		else if (newPosition.x !== this.position.x) newPosition.x = this.position.x
		if (!this.game.ball.passedPaddleLine) {
			this._position = newPosition
			return
		}
		const newRect = super.getRectFromOffsetAndPos(newPosition, Paddle.xOffset, Paddle.yOffset)
		...
		if (
			((isUp && isBallUpper) || (!isUp && !isBallUpper)) &&
			((isUp && newRect.topY < topPaddleLimit) || (!isUp && newRect.botY > botPaddleLimit))
		) {
			return
		}
		const paddleCollideWithBall = this.doesBallCollideWithPaddle(
			this.game.ball.getRect(),
			newRect,
		)
		...
		const newBallY = isUp ? newRect.topY - Ball.offset - 1 : newRect.botY + Ball.offset + 1
		this.game.ball.position.y = newBallY
		this._position = newPosition
	}

	public update(deltaTime: number) {
		if (this._movement === "NONE") return
...
class Ball extends GameObject {
	...
	private doesBallCollideWithLeftWall = (ball: Rectangle) => ball.leftX <= 0

	private doesBallCollideWithRightWall = (ball: Rectangle) => ball.rightX >= GameDim.court.width
	...
	private move(dist: number) {
		...
		const nextPosRect: Rectangle = super.getRectFromOffsetAndPos(
			nextPosWithoutColl,
			Ball.offset,
			Ball.offset,
		)

		const facingPaddle: Paddle =
			this.direction.x > 0 ? this.game.playerB.paddle : this.game.playerA.paddle
		let collideWithPaddles =
			this.doesBallCollideWithPaddle(nextPosRect, this.game.playerA.paddle.getRect()) ||
			this.doesBallCollideWithPaddle(nextPosRect, this.game.playerB.paddle.getRect())
		const collideWithTopBotWalls = this.doesBallCollideWithTopBotWalls(nextPosRect)

		if (
			!collideWithPaddles &&
			dist > GameDim.paddle.width &&
			...
			if (
				intersecY + Ball.offset >= facingPaddle.getRect().topY &&
		...
		const yWall = this.direction.y > 0 ? GameDim.court.height - Ball.offset : Ball.offset
		const xPaddle =
			this.direction.x > 0
				? GameDim.court.width - GameDim.paddle.width - Ball.offset
				: GameDim.paddle.width + Ball.offset
		...
		if (collideWithPaddles) {
			...
			if (this.passedPaddleLine) {
				const yPaddle =
					facingPaddle.getRect().topY >= this.getRect().botY
	...
	public update(deltaTime: number) {
		...
		if (this.speedIncrType === "linear") this.speed += this.linearSpeedIncr * deltaTime
		else this.speed *= 1 + this.exponentialSpeedIncr * deltaTime
	}
}
...
class Game {
	...
	constructor(
		...
		this.webSocket.server.sockets
			.in([clientA.data.intraUserName, clientB.data.intraUserName])
			.socketsJoin(this.id)
		...
		this.webSocket.server.on("connect", (client) => {
			if (!this.playerA.isPause() && !this.playerB.isPause()) return
			if (
				client.data.intraUserName !== this.playerA.user.intraUserName &&
			...
			player.unpause()
			if (!this.playerA.isPause() && !this.playerB.isPause()) this.status = "PLAY"
		})
	}

	private async clientListen(intraUserName: IntraUserName) {
		this.webSocket.server.sockets.adapter.on("delete-room", (roomId) => {
	...
	private getPlayerByIntraUserName(intraUserName: IntraUserName) {
		return this.playerA.user.intraUserName === intraUserName ? this.playerA : this.playerB
	}

	public updateMovement(intraUserName: IntraUserName, move: GameMovement) {
		this.getPlayerByIntraUserName(intraUserName).paddle.movement = move
	}

	private callOnAllGameObjects<
		Key extends {
			[K in keyof GameObject]: GameObject[K] extends (...args: any[]) => void ? K : never
		}[keyof GameObject],
	>(key: Key, ...args: Parameters<GameObject[Key]>) {
		const gameObjects: GameObject[] = [this.playerA.paddle, this.playerB.paddle, this.ball]
	...
	public surrend(intraUserName: IntraUserName) {
		this.getPlayerByIntraUserName(intraUserName).score = this.maxScore
		this.handleWin()
	}
...
export class GameService {
	...
	constructor(
		...
		setTimeout(
			(() => {
				this.webSocket.server.on("connect", this.connectUser.bind(this))
			}).bind(this),
			0,
		)
		setTimeout(
	...
	public disconnectUser({ intraUserName }: EnrichedSocket["data"]) {
		if (this.queue?.data.intraUserName === intraUserName) this.queue = null
	}
	...
	private loop() {
		this.games.forEach((game) => {
			if (game.status === "PLAY") game.update()
		})
	...
	public queueUser(client: EnrichedSocket) {
		if (this.queue?.data.intraUserName === client.data.intraUserName) return
		if (!this.queue) {
			this.queue = client
			return
		}
