...
export class UserService {
    ...
    private async formatUserStatusForUser(username: string, toGetStatusUserName: string)
    ...
    test(username: string) {
        return {
            ...
            Record<
                keyof Omit<Extract<RequestShapes['searchUsers']['query']['filter'], { type: "inc" }>, "type">,
                Prisma.Enumerable<Prisma.UserWhereInput>
            > & Record<
                keyof Omit<Extract<RequestShapes['searchUsers']['query']['filter'], { type: "only" }>, "type">,
                Prisma.Enumerable<Prisma.UserWhereInput>
            >
    }

    getTest(arg: Omit<RequestShapes['searchUsers']['query']['filter'], "type">, type: "inc" | "only",username: string) {
    ...
    async searchUsers(username: string, contains: string, nRes: number, filter: RequestShapes['searchUsers']['query']['filter']) {
        const where = {
            name: { contains },
    ...
    getArrayOfUniqueUserNamesFromStatusData(data: Awaited<ReturnType<typeof this.getNotifyStatusData>>) {
        return [...new Set<string>(data.friend.map(el => el.requestedUserName)
        ...
        return this.getUserByNameOrThrow(username, {
            ...
            directMessage: {
                where: {
                    ...
                    OR: [ {
                            OR: [
                                { requestedUser: { friend: { some: { requestedUserName: username } } } },
                                { requestedUser: { friendOf: { some: { requestingUserName: username } } } },
                            ]
                        }, {
                            requestingUser: { statusVisibilityLevel: "IN_COMMON_CHAN" },
                            requestedUser: { chans: { some: { users: { some: { name: username } } } } }
                        }, {
            ...
            directMessageOf: {
                where: {
                    ...
                    OR: [ {
                            OR: [
                                { requestingUser: { friend: { some: { requestingUserName: username } } } },
                                { requestingUser: { friendOf: { some: { requestedUserName: username } } } },
                            ]
                        }, {
                            requestedUser: { statusVisibilityLevel: "IN_COMMON_CHAN" },
                            requestingUser: { chans: { some: { users: { some: { name: username } } } } }
                        }, {
