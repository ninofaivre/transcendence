...
export class FriendInvitationsController implements NestControllerInterface<typeof c>
{
	...
	async updateFriendInvitation(@Request()req: EnrichedRequest, @TsRestRequest(){ body: { status }, params: { id } }: RequestShapes['updateFriendInvitation'])
	{
		const body = await this.friendInvitationsService.updateFriendInvitation(req.user.username, status, id)
		return { status: 200 as const, body }
	}
}
